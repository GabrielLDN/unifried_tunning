VehicleHandler = function(currentVehicle, vehiclePlate, vehicleModel)
    ...
    RegisterNUICallback("handleMenuClick", function(data, cb)
        local action = data.action
        if action == "engine" then
            ExecuteEngineSettings() -- Função que você irá definir para lógica específica
        else
            print("Ação desconhecida recebida:", action)
        end
        cb("ok")
    end)
    
    -- Função para executar as configurações do motor
    function ExecuteEngineSettings()
